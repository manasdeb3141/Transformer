...
from get_QKV import get_self_atten_QKV_head
from get_attention_scores import get_self_attention_scores
...
def main():
    ...
    save_file = Path(f"data/self_QKV_head_mi_attn_scores_epoch_{epoch}_layer_{attention_layer}_head_{head_id}_sentence_{sentence_id}.pt")
    ...
    if save_file.exists():
        ...
        QKV_head_database = get_self_atten_QKV_head(analyzer, sentence_id, decoder_token_id)
        attention_scores = get_self_attention_scores(analyzer, sentence_id, head_id, decoder_token_id, attention_layer)
