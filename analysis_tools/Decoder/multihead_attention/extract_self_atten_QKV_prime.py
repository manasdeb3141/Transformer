...
def extract_self_atten_QKV_prime(epoch, attention_layer, sentence_id, decoder_token_id):
...
def main():
    ...
    max_tokens, tgt_mask_size = get_max_tokens(epoch, attention_layer, sentence_id)
    print(f"Max tokens: {max_tokens}, Target mask size = {tgt_mask_size}")

    token_list = np.arange(10, max_tokens)

    for decoder_token_id in tqdm(token_list.tolist()):
        extract_self_atten_QKV_prime(epoch, attention_layer, sentence_id, decoder_token_id)
