...
def plot_FF_mi(FF_mi, epoch, decoder_layer, sentence_id):
    ...
    fig.suptitle(f"Mutual Information between the input and output of the FF network\nfor epoch {epoch}, attention layer {decoder_layer}, sentence {sentence_id}")
    plt.show(block=True)


def compute_FF_mi(analyzer, FF_inout, decoder_layer, sentence_tokens):
...
def main():
    ...
    save_file = Path(f"data/ff_mi_epoch_{epoch}_layer_{decoder_layer}_sentence_{sentence_id}_token_{decoder_token_id}.pt")
    ...
    if save_file.exists():
        ...
        FF_mi = compute_FF_mi(analyzer, FF_inout, decoder_layer, tgt_sentence_tokens)

        # Save the file
        torch.save(FF_mi, save_file)
    
    plot_FF_mi(FF_mi, epoch, decoder_layer, sentence_id)
