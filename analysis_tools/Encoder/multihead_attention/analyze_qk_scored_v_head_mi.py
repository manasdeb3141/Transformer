...
def compute_QK_scored_V_head_mi(analyzer, QKV_head_database, head_id, attention_layer, sentence_tokens):
    ...
    V_heads = QKV_dict["value"]
    scored_V_heads = QKV_dict["scored_value"]
...
def main():
    print("Computing the mutual information betwwen the Q, K attention heads and the scored V head ...")
    ...
    save_file = Path(f"data/QK_scored_V_head_mi_epoch_{epoch}_head_{head_id}_layer_{attention_layer}_sentence_{sentence_id}.pt")
    ...
    if save_file.exists():
        ...
        QK_scored_V_head_mi = compute_QK_scored_V_head_mi(analyzer, QKV_head_database, head_id, attention_layer, src_sentence_tokens)
